<script lang="ts">
    ...
    function updateNodeState(updateFn: (state: NodeState) => void) {
        let _nodeStates = get(nodeStates);
        let nodeState = _nodeStates[pathStr];
        if (!nodeState) {
            nodeState = defaultNodeState;
            _nodeStates[pathStr] = nodeState;
            nodeStates.set(_nodeStates);
        }
        updateFn(nodeState);

        // quick n dirty comparison
        if (JSON.stringify(nodeState) == defaultNodeStateJson) {
            delete _nodeStates[pathStr];
            nodeStates.set(_nodeStates);
        }
        return state;
    }
    ...
    $: childrenStart,
        updateNodeState((state) => (state.childrenStart = childrenStart));
    ...
    function nodeOnSelect(unselectOthers: boolean): boolean {
        if (node.type == "entry") {
            ...
            let _selectedNodes = get(selectedNodes);
            if (pathStr in _selectedNodes) {
                delete _selectedNodes[pathStr];
                selectedNodes.set(_selectedNodes); // force update
                return false;
            }

            _selectedNodes[pathStr] = node.content.length;
            selectedNodes.set(_selectedNodes);

            currentPath.set(path);
            currentTextSlots.set(node.content);
            currentNav.set({
                next: node.next,
                prev: node.prev,
            });
    ...
    function onClick(e: PointerEvent) {
        ...
        document.addEventListener(
            ...
            () => {
                if (justFocused) {
                    document.addEventListener("click", () => (focus = false), {
                        once: true,
                    });
                    return;
                }
    ...
    function onMouseMove(e: MouseEvent) {
        if (e.ctrlKey && e.buttons == 1 && !(pathStr in get(selectedNodes))) {
            nodeOnSelect(false);
        }
    }
    ...
    let onMessage = (e: MessageEvent<ControllerMessage>) => {
        ...
        if (isEntry) {
            if (
                message.type == "setExists" &&
                message.path.join("/") == pathStr
            ) {
                noContent = !message.exists;
            }
        } else if (
            message.type == "setCategoryFull" &&
            message.path.join("/") == pathStr
        ) {
